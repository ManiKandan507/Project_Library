...
import { CANADA_API_KEY, CANADA_REGIONS_LIST, CANADA_VIEW, CONTINENTS_BASED_COUNTRIES, CONTINENT_VIEW, COUNTRY_VIEW, REGION_VIEW, STATE_VIEW, USA_API_KEY, USA_REGIONS_LIST, USA_VIEW } from "@/CorporateMembershipReportingV2/LocationReport/constants";
import GEO_COUNTRIES from "@/CorporateMembershipReportingV2/LocationReport/geojsons/countries.json";
import GEO_USA from "@/CorporateMembershipReportingV2/LocationReport/geojsons/usa-states.json";
import GEO_CANADA from "@/CorporateMembershipReportingV2/LocationReport/geojsons/canada-states.json";
...
export const hashMapCountryBasedData = (countries) => {
  let mappedCountries = countries.map(country => {
    return ({
      ...country,
      Members: country['CountPerCountry'],
      Continent: (_findKey(CONTINENTS_BASED_COUNTRIES, (o) => o.includes(country["Country"])) ?? 'N/A'),
      key: country["Country"]
    })
  }
  ).filter(e => e.Country);
...
export const hashMapStateBasedData = (states) => {
  ...
  _map(
    _groupBy(states.map(state => ({
      ...
      key: state['State']
    })).filter(e => e.Country && e.State), 'Country'),
    (statesArr, Country) => {
