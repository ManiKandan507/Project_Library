...
import { useRenewMemberHook, useRenewMemberDetailsHook } from '@/CorporateMembershipReportingV2/hooks/Members';
...
const RenderMembersModal = memo(({
    ...
    let { renewMemDetails, renewMemLoading } = useRenewMemberDetailsHook(appdir, groupId, dates[0], dates[1],  detailed, offset, limit, type, signal)
    ...
    return(
        <div>
            {<Modal visible={showModal ? showModal : memberModal} title="Member Details" onCancel={handleCancel} footer={null} width='80%'>
                ...
                <Table dataSource={memberDetails} columns={MemberDetailsColumn} pagination={true} className='CurrentMemberTable' scroll={{y: 300}} loading={renewMemLoading} size="small"/>
            </Modal>}
        </div>
    )
})
...
const RenewMembers = (props) => {
    ...
    let { renewMembers, loading:renewMemberLoading } = useRenewMemberHook(appdir, groupid, dates[0], dates[1], type === RENEW_MEMBERS, signal)
    ...
    const handleChartClick = ({data}) =>{
      ...
      setRenewMemInfo({detailed:1, offset:0, limit:data?.CountPerGroup})
      setShowModal(true)
    }
    ...
    const handleClick = () =>{
        ...
        setRenewMemInfo({detailed:1, offset:0, limit:totalCount})
        setGroupId(groupIds)
        setMemberModal(true)
    }
    ...
    const renderActionBar = () => {
        return (
            <div className='pt-2'>
                <div className='mt-3 mb-3 title' style={{ fontWeight: 100 }}> {"RENEW MEMBERS"} : <a className='menuValues member-count' onClick={handleClick} style={{color:'#faad14'}}> {MemberCount}</a></div>
                <Row gutter={16} className='mb-3 row-gap'>
                    ...
                    {active === "table" ? <Col>
                        ...
                        <Col>
                            <DownloadChart domEl={domEl} fileName={{ name: "Renew Members", startDate: moment(dates[0], 'DD-MM-YYYY').format('MM/DD/YYYY'), endDate: moment(dates[1], 'DD-MM-YYYY').format('MM/DD/YYYY')  }} />
                        </Col>
                    }
                </Row>
            </div>
    ...
    return (
        <CommonSpinner loading={renewMemberLoading} className="initialLoader">
            <MultiSelectWidget 
            ...
            <RenderMembersModal 
                ...
                signal={signal}
                appdir={appdir}
                detailed={renewMemInfo.detailed}
                limit={renewMemInfo.limit}
                offset = {renewMemInfo.offset}
