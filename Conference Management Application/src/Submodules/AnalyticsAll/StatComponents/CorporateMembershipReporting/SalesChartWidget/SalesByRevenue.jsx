...
import { getSalesTableData, getSalesByVolumeTableData, getTotalCount, getDateToConstruct } from '@/CorporateMembershipReportingV2/SalesChartWidget/helper';
...
import { SimpleSalesChart } from "./helper";
...
const SalesbyRevenue = (props) =>{
    ...
    const getColumn = () => {
        ...
        let columns = monthYear?.map((month) => {
            return {
                ...
                className: "text-left",
                render: (_,data) => {
                    let text = currencyFormatter(data[month]?.TotalRevenue);
                    if(!data[month]?.TotalRevenue){
                       return text 
                    }
    ...
    const renderActionBar = () => {
        return (
            <>
               <div className='mt-3' style={{ fontWeight: 100 }}>{"TOTAL REVENUE"} : <a className='menuValues member-count' onClick={handleCountClick}>{currencyFormatter(memberCount)}</a></div>
                <Row gutter={16} className='mt-4 mb-2 row-gap'>
                    ...
                    {active === "table" && tableDataSource.length ? 
                        <Col>
                        <CustomExportCsv
                                dataSource={getSalesByVolumeTableData(tableDataSource).data}
                                Headers={getSalesByVolumeTableData(tableDataSource).columns}
                                exportFileName={'TOTAL REVENUE'}
                            />
                        </Col> :
                        <Col>
                          <DownloadChart domEl={domEl} fileName={{ name:'TOTAL REVENUE', startDate: defaultDates[0].format('MM/DD/YYYY'), endDate: defaultDates[1].format('MM/DD/YYYY')}} />
                        </Col> 
                    }
                </Row>
            </>
