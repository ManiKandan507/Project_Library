...
import { useNewMembersHook, useNewMemberDetails } from '@/MembershipReportingV2/hooks/Members'
...
const RenderMembersModal = memo(({
    ...
    let { newMembersDetails, newMemLoading } = useNewMemberDetails(source_hex, groupIds, dates[0],dates[1],type, signal)
    ...
    return(
         <div>
            {<Modal open={showModal ? showModal : memberModal} title="Member Details" onCancel={handleCancel} footer={null} width='80%'>
                ...
                <Table dataSource={memberDetails} columns={MemberDetailsColumn} pagination={true} className='CurrentMemberTable' scroll={{y: 450}} loading={newMemLoading}/>
            </Modal>}
        </div>
    )
})
...
const NewMembers = (props) => {
    ...
    let { newMembers, loading:newMemberLoading} = useNewMembersHook(source_hex,dates[0],dates[1], type === NEW_MEMBERS, signal)
    ...
    const handleChartClick = ({data}) =>{
      ...
      setMemberData({ GroupName:data?.GroupName,TotalCount:data?.TotalCount, dataSource: newMembers})
      setShowModal(true)
    }
                ...
                <Row gutter={16} className='mb-3 row-gap'>
                    ...
                    {active === "table" && dataSource.length ? <Col>
                        <CustomExportCsv
                            ...
                            Headers={newMembersHeader}
                            exportFileName={"NEW MEMBERS"}
                        />
                    </Col> : ' ' }
                    {active !== 'table' && dataSource.length ?
                        <Col>
                            <DownloadChart fileName={{ name: "Current Members", startDate: moment(dates[0], 'DD-MM-YYYY').format('MM/DD/YYYY'), endDate: moment(dates[1], 'DD-MM-YYYY').format('MM/DD/YYYY') }}  chartRef={chartRef} />
                        </Col> : ''
                    }
                </Row>
            </div>
    ...
    return (
        <CommonSpinner loading={newMemberLoading}>
            <MultiSelectWidget
