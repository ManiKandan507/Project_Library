...
import { CANADA_API_KEY, CANADA_REGIONS_LIST, CANADA_VIEW, CONTINENTS_BASED_COUNTRIES, CONTINENT_VIEW, COUNTRY_VIEW, REGION_VIEW, STATE_VIEW, USA_API_KEY, USA_REGIONS_LIST, USA_VIEW } from "../../../constants";
import GEO_COUNTRIES from "./geojsons/countries.json";
import GEO_USA from "./geojsons/usa-states.json";
import GEO_CANADA from "./geojsons/canada-states.json";
...
export const hashMapCountryBasedData = (countries) => {
  ...
  console.log('N/A continents', mappedCountries.filter(e => e.Continent === 'N/A'))
...
export const getColumnsConfig = (callback) => {
  let baseArr = {
    [COUNTRY_VIEW]: [{ key: 'Country', left: true }, { key: 'Members', render: (data) => callback(data, true, COUNTRY_VIEW) }, {key: 'Continent', label:"World Region"}],
    [CONTINENT_VIEW]: [{ key: 'Continent', left: true }, { key: 'Members', render: (data) => callback(data, false, CONTINENT_VIEW) }],
    [`${USA_VIEW}_${STATE_VIEW}`]: [{ key: 'Country', left: true }, { key: 'State' }, { key: 'Members', render: (data) => callback(data, false, `${USA_VIEW}_${STATE_VIEW}`, STATE_VIEW) }],
    [`${USA_VIEW}_${REGION_VIEW}`]: [{ key: 'Region', left: true }, { key: 'Members', render: (data) => callback(data, false, `${USA_VIEW}_${REGION_VIEW}`, REGION_VIEW) }],
    [`${CANADA_VIEW}_${STATE_VIEW}`]: [{ key: 'Country', left: true }, { key: 'State' }, { key: 'Members', render: (data) => callback(data, false, `${CANADA_VIEW}_${STATE_VIEW}`, STATE_VIEW) }],
    [`${CANADA_VIEW}_${REGION_VIEW}`]: [{ key: 'Region', left: true }, { key: 'Members', render: (data) => callback(data, false, `${CANADA_VIEW}_${REGION_VIEW}`, REGION_VIEW) }],
  }
  let returnObj = {};
  _map(baseArr, (childArr, viewKey) => {
    returnObj[viewKey] = childArr.map(({ key, render = false, left = false }) => ({
      label: key,
      title: <div className={left ? 'text-left' : 'text-center'}>{key === 'Continent' ? 'World Region' : key}</div>,
      dataIndex: key,
      key: key,
...
export const constructSelectedItem = (data, view) => {
  switch (view) {
    case COUNTRY_VIEW: {
      return ({ value: data.Members, country: data.Country, continent: data.Continent })
    }
    case CONTINENT_VIEW: {
