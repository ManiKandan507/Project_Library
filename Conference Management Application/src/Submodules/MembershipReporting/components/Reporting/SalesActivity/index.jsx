...
const SalesActivity = props => {
  ...
  const handleMemberValue = (data, key) => {
    ...
    if (data[key].length > 0) {
      ...
      dispatch(getMemberDetailsRequest({ screen: props.screen, sourceHex, uuids: data[key].slice(0, PAGE_SIZE) }))
    }
  }
  ...
  useEffect(() => {
    if (!_isEmpty(filteredBarChartData)) {
      let totalMembers = 0;
      let label = groupByChart === SALES_BY_VOLUME ? "Total Volume" : "Total Revenue";
  ...
  const handleClick = (val, legend = false) => {
    ...
    if (uuids.length > 0) {
      ...
      dispatch(showModal(false))
      dispatch(getMemberDetailsRequest({ screen: props.screen, sourceHex, uuids: uuids.slice(0, PAGE_SIZE) }))
    }
  }
  ...
  const handleTotalLabelClick = (date) => {
    ...
    if (clickedBarData?.allUUIDs?.length) {
      ...
      dispatch(showModal(false))
      dispatch(getMemberDetailsRequest({ screen: props.screen, sourceHex, uuids: clickedBarData.allUUIDs.slice(0, PAGE_SIZE) }))
    }
  }
